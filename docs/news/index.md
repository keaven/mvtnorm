...
- Added diagnostic tolerance handling for deterministic backends.
  Positive `abseps` or `releps` requests are checked against
  deterministic stability diagnostics and return `inform = 1` if the
  requested diagnostic target is not met.
...
- The deterministic `error` attribute is a reproducible stability
  diagnostic, not a Monte Carlo confidence interval and not a rigorous
  deterministic error bound.
- The current Sobol implementation uses an embedded plain C
  direction-number table rather than modern high-dimensional Joe-Kuo
  direction files.
- The `metal` backend currently probes for a macOS Metal device but
  falls back to C/FORTRAN because no Metal evaluator is implemented yet.
- `backend = "auto"` currently uses fixed deterministic sequence
  offsets. A future implementation should consider stronger
  deterministic digital shifts and more mature adaptive refinement.
